# Example: Continuous optimization using scikit optimize

This toy example demonstrates continuous optimization using scikit optimize and law workflows.
It is based on the example of parallel optimization.
It differs in a way that the resources do not idle, waiting for the other "parallel" jobs to finish.

A number of workers are working simultaneaously. Every time a worker finished with one point told by the optimizer, it directly asks for the next point.
The communication of the workers is realised in the FileTarget of the pickled optimizer.
